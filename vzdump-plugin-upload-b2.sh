...
if [ "$1" == "backup-end" ]; then
  ...
  echo "ENCRYPTING"
  cd "$SECONDARY"
  ls -1 $TARBASENAME.split.* | time xargs --verbose -I % -n 1 -P $NUM_PARALLEL_GPG $GPG_BINARY --no-tty --compress-level 0 --passphrase-file $GPG_PASSPHRASE_FILE --batch -c --output "$DUMPDIR/%.gpg" "%"
  if [ $? -ne 0 ] ; then
